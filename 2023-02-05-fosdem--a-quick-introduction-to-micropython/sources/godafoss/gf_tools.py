...
def bar_bits(
    ...
    """
    int value with (only) the lower n_ones bits 1

    :param n_bits: int
        the number of 1-value bits in the result

    :result: int
        int value, with (only) the lower n_bits bits 1
...
def int_from_bytes(
    ...
    :param array: bytes | byetarray
        the array of bytes that is to be converted to an integer

    :result: int
        the array interpreted as (unsigned) integer value
